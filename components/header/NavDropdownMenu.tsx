...
const NavDropdownMenu = ({ option }: { option: NavSubmenu }) => {
    const [showSubMenu, setShowSubMenu] = useState(false);


    return (
        <div>
            <div onClick={() => setShowSubMenu(prev => !prev)} className={cn("flex items-center gap-1 py-1.5 text-base font-medium text-foreground hover:text-[rgb(var(--bg-primary-hover))] active:text-[rgb(var(--bg-primary-hover))] transition-colors duration-200 ease-in-out cursor-pointer group outline-none",
            ...
            <div className={cn("grid pl-2 border-l border-[rgb(var(--border-default))] transition-all duration-250 ease-in-out",
