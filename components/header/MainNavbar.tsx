                    ...
                    {navOptions.map(option => (
                        <li key={option.id} className="flex items-center font-sans">
                            {option.type !== "Submenu" ? (
                                <Link
                                    href={option.url}
                                    className="px-3 py-1.5 text-base font-medium text-foreground hover:text-[rgb(var(--bg-primary-hover))] active:text-[rgb(var(--bg-primary-hover))] transition-colors duration-250 ease-in-out"
                                >
                                    {option.name}
                                </Link>
                            ) : (
